 ...
 */
const AboutDialog = ({ enabled, floatingStyle, modal, draggable, onClose }) => (
    <Dialog
        style={{
            zIndex: 1992,
    ...
    >
        <span role="header">
            <span className="settings-panel-title">A propos</span>
            <button onClick={onClose} className="settings-panel-close close">
                <Glyphicon glyph="1-close" />
            </button>
        </span>
        <div role="body">
            C'est un test de l'usage de MapStore project avec un simple plugin
            qui vous aide à apprendre au travers d'un exercice à propos de
            MapStore.
        </div>
    </Dialog>
);
...
const About = compose(
    ...
    connect(
        state => ({
            enabled:
                (state.controls &&
                    state.controls.sampleAbout &&
                    state.controls.sampleAbout.enabled) ||
                false,
            withButton: false
        }),
        {
